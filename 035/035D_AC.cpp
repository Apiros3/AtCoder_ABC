        ...
        while(!que.empty()) {
            ...
            for (auto &nv : G[v]) {
                if (ret[v] + nv.second < ret[nv.first]) {
                    ...
                    if (ret[v] == ret[nv.first]) bipartite_t = false;
                }
            }
        }
    }
...
int main()
{
    ...
    ll A[100009];
    rep(i,1,N+1) cin >> A[i];
    vector<vector<pair<ll,ll>>> vecs(N+1), vecb(N+1);
    ll a[100009], b[100009], c[100009];
    rep(i,0,M) {
        cin >> a[i] >> b[i] >> c[i];
        vecs[a[i]].emplace_back(b[i],c[i]);
        vecb[b[i]].emplace_back(a[i],c[i]);
    }
    BFS_W BF(vecs,N+1,1);
    BFS_W BS(vecb,N+1,1);

    vector<ll> v = BF.vec(), v2 = BS.vec();
    rep(i,0,N+1) v[i] += v2[i];
