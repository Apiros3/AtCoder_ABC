...
int main()
{
    ...
    ll cnt[200009], rcnt[200009]; 
    cnt[0] = 0;
    rep(i,1,N) {
        cnt[i] = cnt[i-1] + i*(A[i]-A[i-1]);
    }
    rcnt[N-1] = 0;
    rep(i,1,N) {
        rcnt[N-i-1] = rcnt[N-i] + i*(A[N-i]-A[N-i-1]);
    }

    rep(i,0,Q) {
        ll X; cin >> X;
        ll T = upper_bound(A,A+N,X)-A;
        ll lsum = 0, rsum = 0;
        if (T!=0) lsum = cnt[T-1] + T*(X-A[T-1]);
        if (T!=N) rsum = rcnt[T] + (N-T)*(A[T]-X);
        cout << lsum + rsum << endl; 
    }
