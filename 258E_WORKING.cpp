...
int main()
{
    ...
    ll A[200009];
    A[0] = W[0];
    rep(i,1,N) A[i] = W[i] + A[i-1];

    vector<ll> vec, v2;
    map<ll,ll> mp;
    ll tmp = lower_bound(A,A+N,X%A[N-1]) - A;
    vec.push_back(tmp);
    v2.push_back(X/A[N-1]);
    mp[tmp] = 1;

    ll looplength, loopstart, cnt = 1;

    while(1) {
        cnt++;
        ll tp = lower_bound(A,A+N,X+A[vec.back()]) - A;
        if (tp == N) 
            tp = lower_bound(A,A+N,(X+A[vec.back()])%A[N-1]) - A;
        vec.push_back(tp);
        v2.push_back((X+A[vec.back()])/A[N-1]);
    ...
    cout << endl;
    cout << loopstart << endl << looplength << endl;
    ...
    rep(i,0,Q) {
        ...
        if (K == 0LL) ret = vec[0] + 1LL + v2[0]*N;
        else if (K <= loopstart) {
            ret = (N+vec[K]-vec[K-1])%N;
            ret += v2[K]*N;
        }
        else {
            ll t = (K - loopstart)%looplength;
            t += loopstart;
            if (t == 0LL) t = looplength;
            ret = (N+vec[t]-vec[t-1])%N;
            ret += v2[t]*N;
        }
        if (ret != 0LL) cout << ret;
        else cout << N;
