...
int main()
{
    ...
    vpll va, vb;
    for(auto u : mpa) va.emplace_back(u.second,u.first);
    for(auto u : mpb) vb.emplace_back(u.second,u.first);
    sort(all(va));
    sort(all(vb));

    if (va[0].second == vb[0].second) {
        cout << N + min(va[0].first+vb[1].first,va[1].first+vb[0].first) << endl;
    }
    else {
        cout << N + va[0].first + vb[0].first << endl;
    }
