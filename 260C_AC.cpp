...
ll X, Y;
ll DFSr(ll red, ll redlv);
ll DFSb(ll blue, ll bluelv);

ll DFSr(ll red, ll redlv) {
    if (redlv == 1) return 0;
    return red*(DFSr(red,redlv-1) + DFSb(X,redlv));
}
ll DFSb(ll blue, ll bluelv) {
    if (bluelv == 1) return blue;
    return blue*(DFSr(1,bluelv-1) + DFSb(Y,bluelv-1));
}
