...
map<ll,ll> possible(vector<ll> G) {
    map<ll,ll> mp;
    mp[0] = 0;
    for(auto u : G) {
        map<ll,ll> tmp;
...
int main()
{
    ...
    ll cnt = 0, curr = 0, fst = 1;
    ll tm = 0;
    rep(i,0,S.size()) {
        if (S[i] == 'T') {
            if (cnt != 0 && !fst) {
                vec[curr].push_back(cnt);
            }
            if (fst) tm = cnt;
            curr = 1 - curr;
            cnt = 0;
    ...
    map<ll,ll> mp1 = possible(vec[0]), mp2 = possible(vec[1]);

    // for(auto u : mp1) cout << u.first << " ";
    // cout << endl;
    // for(auto u : mp2) cout << u.first << " ";
    // cout << endl;


    if (mp1.count(X-tm) && mp2.count(Y)) {
        cout << "Yes" << endl;
    }
    else {
        cout << "No" << endl;
