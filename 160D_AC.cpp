...
void searc(ll X, ll original, ll dst) {
    if (dst > dist[original][X]) return;
    dist[original][X] = dst;
    if (X!=1) searc(X-1,original,dst+1);
    if (X!=N) searc(X+1,original,dst+1);
    if (X==x) searc(y,original,dst+1);
    if (X==y) searc(x,original,dst+1);
    return;
}
...
int main()
{
    ...
    ll ans[2009];
    rep(i,0,2005) {
        rep(j,0,2005) dist[i][j]=INF;
    }
    rep(i,0,2005) ans[i] = 0;
