...
int main()
{
    ...
    map<ll,ll> mpA, mpB;
    ll hashA[200009], hashB[200009];
    hashA[0] = intpow(2,A[0],mod);
    hashB[0] = intpow(2,B[0],mod);
    mpA[A[0]] = 1; mpB[B[0]] = 1;
    rep(i,1,N) {
        if (mpA[A[i]]) 
            hashA[i] = hashA[i-1];
        else {
            hashA[i] = (hashA[i-1] + intpow(2,A[i],mod))%mod;
            mpA[A[i]]++;
        }
        if (mpB[B[i]]) 
            hashB[i] = hashB[i-1];
        else {
            hashB[i] = (hashB[i-1] + intpow(2,B[i],mod))%mod;
            mpB[B[i]]++;
        }
    }
    ...
    rep(i,0,Q) {
        ll X, Y;
        cin >> X >> Y;
        if (hashA[X-1] == hashB[Y-1]) cout << "Yes" << endl;
        else cout << "No" << endl;
    }
