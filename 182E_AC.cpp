...
{  
    ...
    vector<vector<pair<ll,ll>>> vH(H+1), vW(W+1);
    vector<vector<ll>> vec(H+1,vector<ll>(W+1,0));
    rep(i,1,H+1) vH[i].emplace_back(0,0);
    rep(i,1,W+1) vW[i].emplace_back(0,0);
    rep(i,0,N) {
        cin >> A[i] >> B[i];
        vH[A[i]].emplace_back(B[i],1);
        vW[B[i]].emplace_back(A[i],1);
    }
    rep(i,0,M) {
        cin >> C[i] >> D[i];
        vH[C[i]].emplace_back(D[i],0);
        vW[D[i]].emplace_back(C[i],0);
    }
    rep(i,1,H+1) vH[i].emplace_back(W+1,0);
    rep(i,1,W+1) vW[i].emplace_back(H+1,0);

    rep(i,1,H+1) {
        sort(vH[i].begin(),vH[i].end());
        rep(j,1,vH[i].size()-1) {
            if (vH[i][j].second == 1) {
                rep(k,vH[i][j-1].first+1,vH[i][j+1].first)
                    vec[i][k] = 1;
            }
        }
    }
    rep(i,1,W+1) {
        ...
        rep(j,1,vW[i].size()-1) {
            if (vW[i][j].second == 1) {
                rep(k,vW[i][j-1].first+1,vW[i][j+1].first)
                    vec[k][i] = 1;
            }
        }
    }
    ll cnt = 0;
    rep(i,1,H+1) rep(j,1,W+1) cnt += vec[i][j];
