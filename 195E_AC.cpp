...
string S, X;
vvll dp;
ll memo(ll pos, ll md) {
    if (pos == N) return (md == 0);
    if (dp[pos][md] != -1) return dp[pos][md];
    if (X[pos] == 'T') {
        dp[pos][md] = max(memo(pos+1,md),memo(pos+1,(md+(S[pos]-'0')*modpow(10,N-1-pos,7))%7));
    }
    else {
        dp[pos][md] = min(memo(pos+1,md),memo(pos+1,(md+(S[pos]-'0')*modpow(10,N-1-pos,7))%7));
    }
    return dp[pos][md];
}
