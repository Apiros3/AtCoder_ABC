...
int main()
{
    ...
    ll N; cin >> N;
    ll T[100009], X[100009], A[100009];
    vector<pair<ll,ll>> v(100001,{0,0});
    rep(i,0,N) {
        cin >> T[i] >> X[i] >> A[i];
        v[T[i]] = {X[i],A[i]};
    }
    vector<vector<ll>> vec(100009,vector<ll>(5,-INF));
    vec[0][0] = 0;

    rep(i,1,100001) {
        rep(j,0,5) {
            for(ll k = max(0LL,j-1); k <= min(4LL,j+1); ++k) {
                vec[i][j] = max(vec[i][j],vec[i-1][k]);
            }
        }
