...
int main()
{
    ...
    rep(i,0,N) {
        rep(j,0,N) {
            ...
            ll pow = dist/P[i] + (dist%P[i] != 0);
            vec[i].emplace_back(j,pow);
        }
    }
    ll mn = INF;
    rep(i,0,N) {
        ...
        while(!que.empty()) {
            ...
            que.pop();
            rep(j,0,vec[v].size()) {
                if (max(dist[v],vec[v][j].second) < dist[vec[v][j].first]) {
                    dist[vec[v][j].first] = max(dist[v],vec[v][j].second);
                    que.push(vec[v][j].first);
                }
            }
        }
        ll tmax = 0;
        rep(j,0,N) tmax = max(tmax,dist[j]);
        mn = min(tmax,mn);
    }
    cout << mn << endl;
