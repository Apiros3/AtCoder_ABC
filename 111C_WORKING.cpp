...
int main()
{
    ...
    rep(i,0,N) {
        if (i%2 == 0) {
            ...
            else if (m1 < mp1[V[i]] and mp1[V[i]] < mx1) 
                m1 = mp1[V[i]];
        }
        if (i%2 == 1) {
            mp2[V[i]]++;
            ...
            else if (m2 < mp2[V[i]] and mp2[V[i]] < mx2) 
                m2 = mp2[V[i]];
        }
    }
    if (cn1 == cn2) cout << N - max(mx1+m2, mx2+m1) << endl;
    else cout << N - mx1 - mx2 << endl;
