...
{ 
    ...
    ll N, D, K; cin >> N >> D >> K;
    vll L(D), R(D); input(L,R);
    vll S(K), T(K); input(S,T);
    vll dist(K,INF);

    rep(i,0,D) {
        rep(j,0,K) {
            if (L[i] <= S[j] && S[j] <= R[i]) {
                if (S[j] < T[j]) chmax(S[j],min(T[j],R[i]));
                else chmin(S[j], max(L[i],T[j]));
                if (S[j] == T[j]) chmin(dist[j],i+1);
            }
        }
    }
    for(auto u : dist) cout << u << endl;
