...
vvll adj;
vector<map<ll,ll>> mp, cnt;

ll memo2(ll last, ll cur) {
    if (cnt[last].count(cur)) {
        return cnt[last][cur];
    }
    ll sum = 0;
    for(auto u : adj[cur]) {
...
ll memo(ll last, ll cur) {
    if (mp[last].count(cur)) {
        return mp[last][cur];
    }
    ll sum = 0;
    for(auto u : adj[cur]) {
        if (u != last) {
            sum += memo(cur,u) + memo2(cur,u);
        } 
    }
    mp[last][cur] = sum;
    return sum;
