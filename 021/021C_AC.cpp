...
int main()
{
    ...
    while(!que.empty()) {
        ...
        que.pop();
        rep(i,0,vec[T].size()) {
            if (v[vec[T][i]].first > v[T].first + 1) {
                v[vec[T][i]] = {v[T].first + 1,v[T].second};
                que.push(vec[T][i]);
            }
            else if (v[vec[T][i]].first == v[T].first + 1) {
                v[vec[T][i]].second += v[T].second;
                v[vec[T][i]].second %= mod;
            }
        }
    }
    cout << v[B].second << endl;
