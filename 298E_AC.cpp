...
ll memo(ll a, ll b, ll pos, vector<vvll> &dp) {
    // cout << a << " " << b << " " << pos << endl;
    ...
    ll cnt = 0;
    if (pos == 0) {
        ll modP = modpow(P,MOD9-2,MOD9);
        rep(i,1,P+1) {
            cnt += memo(min(a+i,N),b,1,dp);
        }
        dp[a][b][0] = cnt%MOD9*modP%MOD9;
    }
    else {
        ll modQ = modpow(Q,MOD9-2,MOD9);
        rep(i,1,Q+1) {
            cnt += memo(a,min(b+i,N),0,dp);
        }
        dp[a][b][1] = cnt%MOD9*modQ%MOD9;
    }
    return dp[a][b][pos];
...
int main()
{
    ...
    cout << memo(A,B,0,dp) << endl;
    // for(auto u : dp) {
    //     for(auto v : u) cout << v[0] << " ";
    //     cout << endl;
    // }
