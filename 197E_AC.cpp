...
int main()
{
    ...
    rep(i,1,N) {
        if (vec[i].first != vec[i-1].first) {
            org.emplace_back(bk,vec[i-1].second);
            bk = vec[i].second;
        }
    }
    org.emplace_back(bk,vec[N-1].second);

    ll dp[200009][2];
    dp[0][0] = abs(org[0].second) + abs(org[0].second - org[0].first);
    dp[0][1] = abs(org[0].first) + abs(org[0].second - org[0].first);
    ll T = org.size();
    rep(i,1,T) {
        ll dist = abs(org[i].second - org[i].first);
        dp[i][0] = min(
            dp[i-1][0] + abs(org[i-1].first-org[i].second) + dist,
            dp[i-1][1] + abs(org[i-1].second-org[i].second) + dist
            );
        dp[i][1] = min(
            dp[i-1][0] + abs(org[i-1].first-org[i].first) + dist,
            dp[i-1][1] + abs(org[i-1].second-org[i].first) + dist
            );
    }

    cout << min(
        dp[T-1][0]+abs(org[T-1].first),
        dp[T-1][1]+abs(org[T-1].second)
    ) << endl;
