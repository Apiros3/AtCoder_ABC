...
int main()
{
    ...
    ll sum[5009];
    sum[0] = 0;
    rep(i,1,N+1) sum[i] = sum[i-1] + X[i-1];
    ...
    rep(i,1,vec.size()) vec[i].second += vec[i-1].second;

    vector<ll> cnt(5009,0);

    rep(i,0,N+1) {
        rep(j,0,M) {
            // rep(k,0,N+1) cout << dp[k] << " ";
            // cout << endl;
            // cout << dp[i] << " " << sum[i+1+vec[j].first]-sum[i+1] << " " << vec[j].second << endl;
            if (i+vec[j].first+1 > N) continue;
            // ll T = dp[i+vec[j].first+1];
            dp[i+vec[j].first+1] = max(dp[i+1+vec[j].first],dp[i]+sum[i+1+vec[j].first]-sum[i+1]+vec[j].second);
            // if (T != dp[i+vec[j].first+1]) cnt[i+vec[j].first+1]=1;
        }
    }    
    rep(k,0,N+1) cout << dp[k] << " ";
    cout << endl;
    rep(i,1,N) {
        // if (cnt[i])
        dp[N] = max(dp[N], dp[i] + sum[N]-sum[i]);
    }
    cout << dp[N] << endl;
