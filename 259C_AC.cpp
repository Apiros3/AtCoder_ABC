...
int main()
{
    ...
    vector<pair<ll,ll>> vec, ve2;
    vec.emplace_back(S[0],1);
    ve2.emplace_back(T[0],1);

    rep(i,1,S.length()) {
        if (S[i] == vec[vec.size()-1].first) 
            vec[vec.size()-1].second++; 
        else 
            vec.emplace_back(S[i],1);
    }
    rep(i,1,T.length()) {
        if (T[i] == ve2[ve2.size()-1].first) 
            ve2[ve2.size()-1].second++; 
        else 
            ve2.emplace_back(T[i],1);
    }
    if (vec.size() != ve2.size()) {
        cout << "No" << endl;
        return 0;
    }
    ...
    bool t = 1;
    rep(i,0,vec.size()) {
        if (vec[i].first != ve2[i].first) 
            t = 0;
        if (vec[i].second == 1 and ve2[i].second > 1) 
            t = 0;
        if (vec[i].second > ve2[i].second)
            t = 0;
    }
    if (t) cout << "Yes" << endl;
    else cout << "No" << endl;
