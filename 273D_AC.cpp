...
{ 
    ...
    for(auto &u : mpR) sort(all(u.second));
    for(auto &u : mpC) sort(all(u.second));
    ...
    while(Q--) {
        ...
        if (mpR[RS].size() == 0) {
            mpR[RS].push_back(0);
            mpR[RS].push_back(W+1);
        }
        if (mpC[CS].size() == 0) {
            mpC[CS].push_back(0);
            mpC[CS].push_back(H+1);
        }
        if (D == "L") {
            ll tmp = lower_bound(all(mpR[RS]),CS) - mpR[RS].begin() - 1;
            // cout << tmp << endl;
            CS = max(mpR[RS][tmp]+1, CS-L);
        }
        else if (D == "R") {
            ll tmp = lower_bound(all(mpR[RS]),CS) - mpR[RS].begin() ;
            CS = min(mpR[RS][tmp]-1, CS+L);
        }
        else if (D == "U") {
            ll tmp = lower_bound(all(mpC[CS]),RS) - mpC[CS].begin() - 1;
            RS = max(mpC[CS][tmp]+1, RS-L);
        }
        else {
            ll tmp = lower_bound(all(mpC[CS]),RS) - mpC[CS].begin();
            // debug(mpC[CS]);
            // cout << tmp << endl;
            RS = min(mpC[CS][tmp]-1, RS+L);
        }
