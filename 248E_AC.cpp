...
int main()
{
    ...
    //for Ax + By = C, map <a,b,c> = n;
    ll ans = 0;
    map<tuple<ll,ll,ll>,ll> mp;
    rep(i,0,N) {
        rep(j,i+1,N) {
            ll B = -(vec[j].first - vec[i].first);
            ll A = vec[j].second - vec[i].second; 
            ll C = A*vec[j].first + B*vec[j].second; 
            ...
            ll cn = 0;
            rep(k,0,N) 
                if (A*vec[k].first + B*vec[k].second == C) cn++;
            if (cn >= K) {
                if (mp.count({A,B,C}) == 0) {
                    mp[{A,B,C}] = 1;
                    ans++;
