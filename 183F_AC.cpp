...
{ 
    ...
    while(Q--) {
        ...
        if (K == 1) {
            ...
            if (cnt[UF.root(X)] > cnt[UF.root(Y)]) {
                swap(X,Y);
            }

            for(auto u : vec[UF.root(X)]) {
                vec[UF.root(Y)][u.first] += u.second;
            }
            cnt[UF.root(Y)] += cnt[UF.root(X)]; 

            UF.unite(Y,X);

        }
        else {
            if (!vec[UF.root(X)].count(Y)) cout << 0 << endl;
            else cout << vec[UF.root(X)][Y] << endl;
        }
