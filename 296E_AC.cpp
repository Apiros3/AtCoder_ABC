...
struct LCA {
    ...
    int query(int u, int v) {
        ...
        //find lca via pseudo binary search
        if (u == v) return u;
        rrep(k,K-1,0) {
    ...
    void dfs(vvll &G, int v, int p, int d) {
        parent[0][v] = p;
        dist[v] = d;
        for (auto e : G[v]) {
...
{ 
    ...
    general_dfs GD(vec);
    auto ret = GD.scc_general();

    for(auto u : ret) {
        if (u.size() >= 2) cnt += u.size();
    }
    cout << cnt << endl;
