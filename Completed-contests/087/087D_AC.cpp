...
int main()
{
    ...
    ll N, M; cin >> N >> M;
    ll L[200009], R[200009], D[200009];
    vector<vector<pair<ll,ll>>> vec(N+1);

    rep(i,0,M) {
        cin >> L[i] >> R[i] >> D[i];
        vec[L[i]].emplace_back(R[i],D[i]);
        vec[R[i]].emplace_back(L[i],-D[i]);
    }
    queue<ll> que;
    ...
    rep(i,0,N) {
        while(!que.empty()) {
            ...
            rep(j,0,vec[T].size()) {
                if (dist[vec[T][j].first] == INF) {
                    dist[vec[T][j].first] = dist[T] + vec[T][j].second;
                    que.push(vec[T][j].first);
                }
                else if (dist[vec[T][j].first] != dist[T] + vec[T][j].second) {
                    cout << "No" << endl;
                    return 0;
                }
            }
