...
bool tst(string S) {
    ...
    string tmp = "AGC";
    rep(k,0,4) {
        if (k != 0) swap(S[k],S[k-1]);
        rep(i,0,2) {
            ll tm = 1;
            rep(j,0,3) if (S[i+j] != tmp[j]) tm = 0;
            if (tm) ret = 0;
        }
        if (k != 0) swap(S[k],S[k-1]);
    }
...
int main()
{
    ...
    vector<string> nb = {"AGC","ACG","GAC"};
    
    ll dp[109][64];
    rep(i,0,109) rep(j,0,64) dp[i][j] = 0;
    rep(i,0,64) {
        dp[3][i] = 1;
    }
    rep(i,0,nb.size()) {
    ...
    rep(i,4,N+1) {
        rep(j,0,64) {
            rep(k,0,4) {
                string tm = S[k] + rconv(j);
                if (tst(tm)) {
                    tm.pop_back();
                    dp[i][j] += dp[i-1][conv(tm)];
                }
                // else cout << tm << endl;
            }
            dp[i][j] %= mod;
