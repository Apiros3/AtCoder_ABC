...
// pow in int, takes mod and returns in O(log(topn))
    ll intpow(ll btmn, ll topn, ll modn) {if (topn < 0) return 0; ll ret_num = 1; btmn%=modn; for(; topn; topn/=2, btmn=(btmn*btmn)%modn) if (topn & 1) ret_num=(ret_num*(btmn%modn))%modn; return ret_num%modn;}
...
int main()
{
    ...
    ll N; cin >> N;
    ll ret = 0, T = N;
    for(ll cnt = 1; cnt <= 9; ++cnt) {
        ll mod = intpow(10,cnt,INF);
        ret += T/mod*intpow(10,cnt-1,INF);
        ll lft = T%mod + 1 - intpow(10,cnt-1,INF);
        ret += min(max(0LL,lft),intpow(10,cnt-1,INF));
    }
    cout << ret << endl;
