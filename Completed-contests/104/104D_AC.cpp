...
{ 
    ...
    // debug(A), debug(B), debug(C), debug(Q);
    ll sum = 0;
    rep(i,0,N) {
        if (S[i] == 'B' || S[i] == '?') {
            ll ql = Q[i], qr = Q[N] - Q[i+1];
            ll tl = A[i] * modpow(3,ql,MOD10);
            if (ql > 0) tl += ql * modpow(3,ql-1,MOD10);
            ll tr = (C[N] - C[i+1]) * modpow(3,qr,MOD10);
            if (qr > 0) tr += qr * modpow(3,qr-1,MOD10);
