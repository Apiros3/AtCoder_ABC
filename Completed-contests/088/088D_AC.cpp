...
string S[59];
vector<ll> BFS() {
    vector<ll> ret(H*W,INF);
    ret[0] = 0;
    queue<pair<ll,ll>> que;
    que.push({0,1});
    ll Tl[] = {0,-1,0,1}, Tr[] = {1,0,-1,0};
    while(!que.empty()) {
        ll T1 = que.front().first, T2 = que.front().second;
        que.pop();
        rep(i,0,4) {
            ll rw = T1/W + Tl[i], cl = T1%W + Tr[i];
            if (rw == -1 or H == rw or cl == -1 or W == cl) continue;
            if (T2 >= ret[rw*W+cl]) continue;
            if (S[rw][cl] == '#') continue;
            que.push({rw*W + cl, T2+1});
            ret[rw*W+cl] = T2;
        }
    }
    return ret;
...
int main()
{
    ...
    if (v[H*W-1] == INF) cout << -1 << endl;
    else cout << H*W - cnt - v[H*W-1] - 1 << endl;
