...
int main()
{
    ...
    queue<ll> que;
    que.push(1);
    vector<ll> prev(N+1), dist(N+1,INF);
    dist[1] = 0;
    prev[1] = 0;
    while(!que.empty()) {
        ll T = que.front();
        que.pop();
        rep(i,0,vec[T].size()) {
            if (dist[vec[T][i]] > dist[T]+1) {
                que.push(vec[T][i]);
                dist[vec[T][i]] = dist[T]+1;
                prev[vec[T][i]] = T;
            }
        }
    }
    ...
    ll T1 = v[(v.size()-1)/2], T2 = v[(v.size()-1)/2+1];
    ...
    while(!que.empty()) {
        ...
        que.pop();
        rep(i,0,vec[T].size()) {
            if (T == T1 and vec[T][i] == T2) continue;
            if (T == T2 and vec[T][i] == T1) continue;
            if (dist[vec[T][i]] == INF) {
                que.push(vec[T][i]);
                dist[vec[T][i]] = 0;
            }
