...
int main()
{
    ...
    ll N; cin >> N;
    primesearch PR(N);
    vector<ll> v = PR.retvec();
    
    ll cnt = 0;
    for (ll i = 0; v[i]*v[i]*v[i] < N; ++i) {
        for (ll j = i+1; v[i]*v[j]*v[j] < N; ++j) {
            for (ll k = j+1; v[i]*v[j]*v[k] <= N; ++k) {
                cnt++;
            }
        }
    }
    for (ll i=0; v[i]*v[i]*v[i]*v[i] < N; ++i) {
        for (ll j = i+1; v[i]*v[i]*v[i]*v[j] <= N; ++j) {
            cnt++;
        }
    }
    cout << cnt << endl;
