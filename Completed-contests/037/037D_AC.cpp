...
int main()
{
    ...
    rep(i,0,1009) rep(j,0,1009) A[i][j] = 0;
    rep(i,1,H+1) rep(j,1,W+1) cin >> A[i][j];

    rep(i,0,1009) rep(j,0,1009) ret[i][j] = 1;
    
    vector<pair<ll,ll>> vec;
    rep(i,1,H+1) {
        rep(j,1,W+1) {
            vec.emplace_back(A[i][j],(i-1)*W+j-1);
        }
    }
    ll vx[] = {0,1,0,-1}, vy[] = {1,0,-1,0};
    sort(vec.begin(),vec.end());
    rep(i,0,vec.size()) {
        auto T = vec[i];
        ll val = T.first, posH = T.second/W+1, posW = T.second%W+1;
        rep(j,0,4) {
            if (val < A[posH+vx[j]][posW+vy[j]]) {
                ret[posH+vx[j]][posW+vy[j]] += ret[posH][posW];
                ret[posH+vx[j]][posW+vy[j]] %= mod;
            }
        }
    }
    ll cn = 0;
