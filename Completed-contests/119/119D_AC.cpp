...
int main()
{
    ...
    while(Q--) {
        ll X; cin >> X;
        ll tS = lower_bound(all(S),X)-S.begin();
        ll tT = lower_bound(all(T),X)-T.begin();
        
        ll mn = max(S[tS],T[tT])-X;
        chmin(mn, X-min(S[tS-1],T[tT-1]));
        chmin(mn, abs(S[tS]-T[tT-1])+min(abs(S[tS]-X),abs(T[tT-1]-X)));
        chmin(mn, abs(S[tS-1]-T[tT])+min(abs(S[tS-1]-X),abs(T[tT]-X)));
        cout << mn << endl;
    }
