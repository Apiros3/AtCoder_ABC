...
int main()
{
    ...
    rep(i,D+1,H*W+1) {
        ll tmp = i%D + D*(i%D == 0);
        auto p1 = vec[i-D], p2 = vec[i];
        v.push_back(
            v[v.size() - D] + abs(p1.first-p2.first) + abs(p1.second-p2.second)
        );
    } 
    // rep(i,0,vec.size()) cout << "{" << vec[i].first << ", " << vec[i].second << "} ";
    // cout << endl; 
