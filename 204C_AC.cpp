...
void sear(ll num, ll *str) {
    if (str[num] == 1) return;
    str[num] = 1;
    for (ll i=0; i<vec.at(num).size(); ++i) 
        sear(vec.at(num).at(i),str);
    return;
}
...
int main()
{
    ll str[2009];
    p = str;
    ll N, M, A[2009], B[2009];
    cin >> N >> M;
    vec.resize(N+1);
    rep(i,0,N+1) vec.at(i).resize(0);

    rep(i,0,M) {
        cin >> A[i] >> B[i];
        vec.at(A[i]).push_back(B[i]);
    }
    ll ans = 0;
    rep(i,1,N+1) {
        rep(j,1,N+1) str[j] = 0;
        sear(i, str);
        rep(j,1,N+1) ans += str[j];
    }    
