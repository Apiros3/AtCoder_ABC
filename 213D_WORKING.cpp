...
// stoll equivalent in O(|S|)
    ll to_ll(string S) {ll ret_num = 0, multiplier=0; for(ll i=S.length()-1; i>=0; i--, multiplier*=10) ret_num += (S[i]-'0')*multiplier; return ret_num;}
...
// initiate for large nCr calculations
    ll *pfact, *pinv, *pinvfact;
    void nCrinit(const ll size, ll modn, ll *fact, ll *inv, ll *invfact) {pfact = fact; pinv = inv; pinvfact = invfact; fact_arr(size,modn,pfact); inv_el_fact_arr(size,modn,pinv,pinvfact);}
...
int main()
{
    ...
    rep(i,1,N+1) 
        sort(vec.at(i).begin(),vec.at(i).end(),greater<ll>());
        
    vector<ll> v2, mp(200009);
    mp.at(1) = 1;
    v2.push_back(1);
    cout << 1 << " ";
    while(vec.at(1).size() > 0) {
        ll temp = v2.back();
        if (vec.at(temp).size() == 0) {
            v2.pop_back();
        }
        else {
            while(1) {
                ll temp2 = vec.at(temp).back();
                if (mp.at(temp2) == 1) vec.at(temp).pop_back();
                else {
                    v2.push_back(temp2);
                    mp.at(temp2) = 1;
                    vec.at(temp).pop_back();
                    break;
                }
            }
        }
