...
ll AX, AY, BX, BY; 
ll func(ll Ax, ll Ay, ll Bx, ll By, ll x, ll y) {
    return (Ax-Bx)*(y-Ay) - (Ay-By)*(x-Ax);
}

bool intersect(ll x1, ll y1, ll x2, ll y2) {
    ll s = func(AX, AY, BX, BY, x1,y1);
    ll t = func(AX, AY, BX, BY, x2,y2);
    if (s*t > 0) return 0; 
    s = func(x1, y1, x2, y2, AX, AY);
    t = func(x1, y1, x2, y2, BX, BY);
    if (s*t > 0) return 0;
    return 1;
}
...
int main()
{
    ...
    ll cn = 0;
    rep(i,1,N+1) {
        cn += intersect(X[i],Y[i],X[i-1],Y[i-1]);
    }
