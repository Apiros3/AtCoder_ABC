...
{
    ll N, M, Q; cin >> N >> M >> Q;
    vector<ll> csuml, csumr;
    for(int i = 0; i < N; ++i) {
        ll L, R; cin >> L >> R;
        csuml.push_back(L);
        csumr.push_back(R+1);
    }
    sort(csuml.begin(), csuml.end());
    sort(csumr.begin(), csumr.end());
    while(Q--) {
        ll X, Y; cin >> X >> Y;
        ll t1 = upper_bound(csuml.begin(),csuml.end(),min(M,Y+X)) - csuml.begin();
        ll t2 = upper_bound(csumr.begin(),csumr.end(),max(0LL,Y-X)) - csumr.begin();
        cout << t1-t2 << endl;
    }
    return 0;
}     
