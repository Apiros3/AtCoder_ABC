...
int main()
{
    ...
    sort(adlt.begin(),adlt.end());

    if (adlt.size()==0 or chld.size()==0) {
        cout << N << endl;
        return 0;
    }
    ...
    rep(i,0,chld.size()) {
        ll X = chld[i]+1;
        ll tmp = lower_bound(adlt.begin(),adlt.end(),X)-adlt.begin();
        tmp = adlt.size()-tmp;
        mx = max(mx, tmp+i+1);
    }
    rep(i,0,adlt.size()) {
        ll X = adlt[i];
        ll tmp = lower_bound(chld.begin(),chld.end(),X)-chld.begin();
        mx = max(mx,tmp+(ll)adlt.size()-i);
