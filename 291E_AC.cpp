...
vvll dp;
vll memo(ll V, vvll &vec, vll &X) {
    if (dp.size() <= V) dp.resize(V+1);
    if (dp[V].size() > 0) return dp[V];
    vll ret(1,X[V]);
    for(auto u : vec[V]) add(ret,memo(u,vec,X));
...
int main()
{
    ...
    general_dfs DF(adj);
    vll ts = DF.topological();
    rep(i,1,ts.size()) {
        if (!mp.count({ts[i-1],ts[i]})) {
            cout << "No" << endl;
            return 0;
        }
    }
