...
ll func(ll Y, ll X) {
    ...
    ret += (X/2)*(X/2)%mod*Y%mod;
    ret += (X/2)*M%mod*((Y*(Y-1)/2)%mod)%mod;
    ret += ((X/2)*(Y/2))%mod;
    if (X%2 == 1) {
        ll tmp = (Y+1)/2%mod;
        ret += X*tmp%mod;
        tmp--;
        ret += 2*M*((tmp*(tmp+1)/2)%mod)%mod;
    }
    return ret%mod;
...
int main()
{
    ...
    while(Q--) {
        ...
        cin >> A >> B >> C >> D;
        ll t = 0;
        t += func(B,D) + func(A-1,C-1);
        t -= func(A-1,D);
        t -= func(B,C-1);
        cout << ((t + 100LL*mod)%mod) << endl;
        // cout << func(B,D) << " " << func(A-1,C-1) << endl;
        // cout << func(A-1,D) << " " << func(B,C-1) << endl;
    }
