...
{ 
    ...
    cin >> N >> M;
    mp.resize((1ll<<N),(vector<vector<ld>>((1ll<<M),vector<ld>((N+M+1),-1))));
    X.resize(N+M), Y.resize(N+M);
    input(X,Y);
    X.push_back(0), Y.push_back(0);

    cout << memo(0,0,N+M) << endl;
    // for(auto u : mp) {
    //     cout << u.first.first.first << " " << u.first.first.second << " " << u.first.second << " " << u.second <<  endl;
    // }
