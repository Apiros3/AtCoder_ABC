...
int main()
{
    ...
    //between K-1 and N-K
    ll arr[509][509];
    rep(i,0,509) rep(j,0,509) arr[i][j] = 0;

    //find arr[K-1][K-1];
    ll fsum = 0;
    rep(i,0,2*K-1) {
        rep(j,abs(K-1-i),2*K-1-abs(K-1-i)) {
            fsum += (S[i][j] == 'x');
        }
    }
    arr[0][0] = fsum;
    rep(i,1,M-2*K+2) {
        ll tmp = arr[0][i-1];
        rep(j,0,2*K-1) {
            tmp -= (S[j][abs(K-1-j)+i-1] == 'x');
            tmp += (S[j][2*K-abs(K-1-j)+i-2] == 'x');
            // cout << "{" << j << ", " << 2*K-abs(K-1-j)+i-2 << "} ";
        }
        // cout << endl;
        arr[0][i] = tmp;
    ...
    rep(i,1,N-2*K+2) {
        rep(j,0,M-2*K+2) {
            ll tmp = arr[i-1][j];
            rep(k,0,2*K-1) {
                tmp -= (S[abs(K-1-k)+i-1][k+j] == 'x');
                tmp += (S[2*K-abs(K-1-k)+i-2][k+j] == 'x');
                // cout << "{" << abs(K-1-k)+i-1 << ", " << k+j << "} ";
            }            
            // cout << endl;
            arr[i][j] = tmp;
        }
