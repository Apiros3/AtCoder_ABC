...
struct Init {
    ...
    ~Init() {
        ...
        cout << "\n     PROGRAM SUCCESSFUL :) \n";
        auto durationAll = duration_cast<microseconds> (stop-startInit);
        auto durationSys = duration_cast<microseconds> (stop-startInp);
        ll t1 = durationAll.count(), t2 = durationSys.count();
        string S1 = "SYSTEM: t = " + to_string((long double)t1/1000);
        string S2 = "INPUT : t = " + to_string((long double)t2/1000);
        rep(i,0,3) {S1.pop_back(); S2.pop_back();}
        S1 += " ms"; S2 += " ms";
        ll len = max(S1.length(),S2.length());
        cout << "+";
        rep(i,0,len+6) cout << '-';
        cout << "+\n|  * " << S1;
        rep(i,0,len-S1.length()+2) cout << ' ';
        cout << "|\n|  * " << S2;
        rep(i,0,len-S2.length()+2) cout << ' ';
        cout << "|\n+";
        rep(i,0,len+6) cout << '-';
        cout << "+\n\n";
    }
