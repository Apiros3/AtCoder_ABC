...
{  
    ...
    vector<vector<ll>> vec(3009,vector<ll>(3009,0));
    rep(i,A[0],B[0]+1) vec[0][i]++;
    rep(i,1,N) {
        vector<ll> vsum(3009,0);
        vsum[1] = vec[i-1][0];
        rep(j,1,3008) vsum[j+1] = vsum[j] + vec[i-1][j]; 
        rep(j,A[i],B[i]+1) {
            vec[i][j] = (vsum[j+1])%mod;
        }
    }
    ll sum = 0;
    rep(i,0,vec[N-1].size()) sum += vec[N-1][i];
    cout << sum%mod << endl;
