...
int main()
{
    ...
    while(Q--) {
        ...
        if (n == 1) {
            ...
            char d;
            cin >> i >> d;
            RQ.update(i-1,1LL<<(d-'a'));
        }
        else {
            ll L, R; cin >> L >> R;
            cout << __builtin_popcount(RQ.query(L-1,R)) << endl;
        }
        // vector<ll> tmp = RQ.ret();
        // rep(i,0,tmp.size()) cout << basechange_str(10,2,to_string(tmp[i])) << " ";
        // cout << endl;
    }
