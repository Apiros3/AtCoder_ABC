...
ll memo(ll h, ll w, vvll &dp) {
    ...
    if (h == H-1) {
        dp[h][w] = -memo(h,w+1,dp) + (vec[h][w+1] == '+' ? 1 : -1);
        return dp[h][w];
    }
    if (w == W-1) {
        dp[h][w] = -memo(h+1,w,dp) + (vec[h+1][w] == '+' ? 1 : -1);
        return dp[h][w];
    }
    ll ta = -memo(h,w+1,dp) + (vec[h][w+1] == '+' ? 1 : -1);
    ll tb = -memo(h+1,w,dp) + (vec[h+1][w] == '+' ? 1 : -1);
    dp[h][w] = max(ta,tb);
    return dp[h][w];
...
int main()
{
    ...
    ll fin = memo(0,0,dp);
    if (fin > 0) cout << "Takahashi" << endl;
    if (fin < 0) cout << "Aoki" << endl;
    if (fin == 0) cout << "Draw" << endl;
